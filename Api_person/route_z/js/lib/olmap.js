!function(){
/**
*supernever 如果功能不好用请到百度开源库重新下载，目前流动的线暂时不能用
*
*/
...
CanvasLayer.prototype = new ol.MyOverlay();
...
util.extend(Layer.prototype, {
    ...
    _calculatePixel: function _calculatePixel() {
        ...
        for (var j = 0; j < data.length; j++) {
            // ll：从data中的经纬度投影到WebMercator
            var ll =ol.proj.transform([Number(data[j].lng),Number(data[j].lat)],'EPSG:4326','EPSG:900913');
            if (ll[0] && ll[1]) {
                data[j].w = (ll[0]-mcCenter[0])/(size[0]);
                data[j].h = (ll[1]-mcCenter[1])/(size[1]);
            }
            ...
            if (data[j].lng && data[j].lat && !data[j].x && !data[j].y) {
                ...
                var pixel =ol.proj.transform([data[j].lng, data[j].lat],'EPSG:4326','EPSG:900913');
                data[j].x = pixel[0];
                data[j].y = pixel[1];
                data[j].px = pixel.x;
                data[j].py = pixel.y;
            ...
            if (ll[0] && ll[1]) {
                //  将data中的坐标点（投影到WebMercator后）减去屏幕左上角的坐标，得到米，再乘以系数转换到相对于屏幕左上角的像素
                data[j].px = Number(ll[0]-extent[0])* pixel_per_m_width;
                // data[j].py = Math.abs(Number(ll[1]-extent[3]))*pixel_per_m_height;
                data[j].py = (Number(extent[3]-ll[1]))*pixel_per_m_height;
            }
        ...
        var zoomUnit = Math.pow(2, 18 - zoom);
        var mcCenter = map.getView().getCenter();
        //var nwMc = new ol.Pixel(mcCenter.x - map.getSize().width / 2 * zoomUnit, mcCenter.y + map.getSize().height / 2 * zoomUnit); //左上角墨卡托坐标
        var nwMc = [mcCenter[0] -0.5971642833709717 * (map.getSize()[0] / 2 * zoomUnit), mcCenter[1] +0.5971642833709717 * (map.getSize()[1] / 2 * zoomUnit)];
        var data = this.getData();
        var map = this.getMap();
        var lonlat = [(nwMc[0]-mcCenter[0]),(nwMc[1]-mcCenter[1])];//一个象限的宽高
        for (var j = 0; j < data.length; j++) {
            var ll = ol.proj.transform([data[j].lng,data[j].lat],'EPSG:4326','EPSG:900913');
             if (ll[0] && ll[1]) {
                 //data[i].px = (data[i].x - nwMc[0]) / zoomUnit;
                 //data[i].py = (nwMc[1] - data[i].y) / zoomUnit;
                 data[j].px = -(ll[0]-mcCenter[0])/(lonlat[0]);
                 data[j].py = -(ll[1]-mcCenter[1])/(lonlat[1]);
             }
            }
            /**
            if (data[j].lng && data[j].lat && !data[j].x && !data[j].y) {
                var pixel = mercatorProjection.lngLatToPoint(new ol.Point(data[j].lng, data[j].lat));
                data[j].x = pixel.x;
                data[j].y = pixel.y;
            ...
            if (data[j].geo) {
                ...
                if (this.getCoordType() === 'bd09ll') {
                    for (var i = 0; i < data[j].geo.length; i++) {
                        var pixel = map.pointToPixel(new ol.Point(data[j].geo[i][0], data[j].geo[i][1]));
                        tmp.push([pixel.x, pixel.y, parseFloat(data[j].geo[i][2])]);
                    }
                } else if (this.getCoordType() === 'bd09mc') {
                    for (var i = 0; i < data[j].geo.length; i++) {
...
function DataRangeControl() {
    ...
    this.defaultOffset = {width: 10, height: 10};
}

DataRangeControl.prototype = new ol.control.defaults();
...
util.extend(DataRangeControl.prototype, {
    ...
    drawCategorySplit: function drawCategorySplit(splitList, drawOptions) {
        if(!document.getElementById('category')){
            ...
            canvas.width = 80;
            canvas.height = 190;
            canvas.style.cssText = "width:80px;height:190px;position:absolute;top:0px;right:0px;background:#fff;";
        }else{
...
DrawScale.prototype = new ol.control.defaults();
...
function DrawTypeControl(options) {
    ...
    this.defaultOffset = this.getDrawTypeControlOptions().offset || new ol.Size(10, 10);
}

util.inherits(DrawTypeControl, Class);
util.inherits(DrawTypeControl, ol.control.defaults());
...
ClusterDrawer.prototype.drawMap = function () {
    ...
    var nwMc = new ol.Pixel(nwMcX, mcCenter.y + map.getSize().height / 2 * zoomUnit);
    // 左上角墨卡托坐标
...
SimpleDrawer.prototype.drawMap = function (time) {
    ...
    if (dataType === 'polyline' || dataType === 'polygon') {
        ...
        if (drawOptions.strokeStyle || drawOptions.globalCompositeOperation) {
            ...
            for (var i = 0, len = data.length; i < len; i++) {
                var item = data[i];
                // if (item.px < 0 || item.px > ctx.canvas.width || item.py < 0 || Math.abs(item.py) > ctx.canvas.height) {
                //     continue;
                // }
                ctx.moveTo(item.px, -Math.abs(item.py));
                if (icon && icon.show && icon.url) {
                    this.drawIcon(ctx, item, icon);
                } else {
                    if (radius < 2) {
